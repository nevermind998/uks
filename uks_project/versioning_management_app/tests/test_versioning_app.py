...
def get_jwt_token():
    client = Client()
    credentials = {"email": "johndoe@test.cm", "password": "123"}
...
class RepositoryApiTests(TestCase):
    ...
    def test_get_repository_collaborators_successful(self):
        collaborator = User.objects.get(username="johndoe")
        response = self.client.get('/versioning/repository/' + str(collaborator) + '/collaborators', HTTP_AUTHORIZATION=self.token, content_type='application/json')
        response_obj = json.loads(response.content)
    ...
    def test_get_repository_collaborators_user_not_found(self):
        response = self.client.get('/versioning/repository/' + '22' + '/collaborators', HTTP_AUTHORIZATION=self.token, content_type='application/json')
        self.assertEqual(response.status_code, 401)
    ...
    def test_create_repository_successful(self):
        ...
        self.assertEqual(response_obj['name'], 'Project Repository')
        self.assertEqual(response_obj['owner'].id, owner)
    ...
    def test_edit_repository_successful(self):
        repository = Repository.objects.get(name="UKS")
        new_repository = {
            "name": "New Repository",
            "owner": repository.owner.id,
            "description": "This is new created repo",
        }
        response = self.client.put('versioning/repository/' + str(repository.id), data=json.dumps(new_repository), HTTP_AUTHORIZATION=self.token, content_type='application/json')
        response_obj = json.loads(response.content)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response_obj['name'], 'New Repository')
        self.assertEqual(response_obj['description'], 'This is new created repo')
    ...
    def test_delete_repository_successful(self):
        repository = Repository.objects.get(name="UKS")
        response = self.client.delete('versioning/repository/' + str(repository.id), data=json.dumps(repository), HTTP_AUTHORIZATION=self.token, content_type='application/json')
